...
const getPartialValueUnsupported = (context, value, browsers) => {
    ...
    for (const entry of Object.values(context)) {
        ...
        if (matches.regex_value && new RegExp(matches.regex_value).exec(unprefixedValue)) {
            return browsers_1.getUnsupportedBrowsers(entry, prefix, browsers);
        }
        if (matches.keywords) {
            for (const [tokenPrefix, tokenValue] of tokens) {
                if (matches.keywords.includes(tokenValue)) {
                    return browsers_1.getUnsupportedBrowsers(entry, tokenPrefix, browsers);
                }
            }
        }
...
const getValueUnsupported = (context, value, browsers) => {
    const [data, prefix] = helpers_1.getFeatureData(context, value);
    if (data) {
        return browsers_1.getUnsupportedBrowsers(data, prefix, browsers);
    }
...
exports.getDeclarationUnsupported = (feature, browsers) => {
    ...
    return cache_1.getCachedValue(key, browsers, () => {
        const [data, prefix] = helpers_1.getFeatureData(browser_compat_data_1.mdn.css.properties, feature.property);
...
exports.getRuleUnsupported = (feature, browsers) => {
    return cache_1.getCachedValue(`css-rule:${feature.rule}`, browsers, () => {
        const [data, prefix] = helpers_1.getFeatureData(browser_compat_data_1.mdn.css['at-rules'], feature.rule);
        return browsers_1.getUnsupportedBrowsers(data, prefix, browsers);
    });
};
const getPseudoSelectorUnsupported = (value, browsers) => {
    ...
    return cache_1.getCachedValue(`css-pseudo-selector:${name}`, browsers, () => {
        const [data, prefix] = helpers_1.getFeatureData(browser_compat_data_1.mdn.css.selectors, name);
        return browsers_1.getUnsupportedBrowsers(data, prefix, browsers);
    });
};
