...
const file_extension_1 = require("./fs/file-extension");
exports.getFileExtension = file_extension_1.default;
const file_name_1 = require("./fs/file-name");
...
const determineCharset = (originalCharset, mediaType) => {
    ...
    let determinedCharset = typeInfo && normalize_string_1.default(typeInfo.charset);
...
const determineMediaTypeForScript = (element) => {
    const typeAttribute = normalize_string_1.default(element.getAttribute('type') || '');
...
const determineMediaTypeBasedOnElement = (element) => {
    const nodeName = element && normalize_string_1.default(element.nodeName);
    if (element && nodeName) {
        if (nodeName === 'script') {
...
const determineMediaTypeBasedOnFileExtension = (resource) => {
    const fileExtension = file_extension_1.default(resource);
    if (!fileExtension) {
        return null;
    }
...
const determineMediaTypeBasedOnFileName = (resource, rawContent) => {
    const fileName = file_name_1.default(resource);
    if (!fileName) {
        return null;
    }
...
const determineMediaTypeBasedOnFileType = (rawContent) => {
    ...
    const detectedFileType = file_type_1.default(rawContent);
    if (detectedFileType) {
...
const parseContentTypeHeader = (headers) => {
    const contentTypeHeaderValue = normalize_string_1.default(headers ? headers['content-type'] : null);
